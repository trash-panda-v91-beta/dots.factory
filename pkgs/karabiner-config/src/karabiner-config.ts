...
function rule_leaderKey() {
  ...
  let mappings = {
    ...
    r: {
      name: 'Raycast',
      mapping: {
        c: ['extensions/raycast/calendar/my-schedule', 'Calendar'],
        d: ['extensions/raycast/dictionary/define-word', 'Dictionary'],
        e: ['extensions/raycast/emoji-symbols/search-emoji-symbols', 'Emoji'],
        s: ['extensions/raycast/snippets/search-snippets', 'Snippets'],
        v: ['extensions/raycast/clipboard-history/clipboard-history', 'Clipboard'],
        b: ['quicklinks/https://google.com/search?q=', 'Google'],
        r: ['script-commands/rebuild-nixos', "Rebuild NixOS"],
  ...
  return rule('Leader Key').manipulators([
    ...
    withCondition(ifVar(_var, 0))([
      mapSimultaneous(['f', 's'], undefined, 250)
        .toVar(_var, 1)
...
function rule_workspaces() {
  let hint = `T: Terminal | B: Browser | N: Notes`
  let layer = duoLayer('f', 'j').notification(hint).description('Open App').leaderMode()
...
function app_homerow() {
  return rule('Homerow').manipulators([
    mapSimultaneous(['f', 'l']).to('s', 'Hyper'), // Click
    mapSimultaneous(['f', 'k']).to('j', 'Hyper'), // Scroll
  ])
}
