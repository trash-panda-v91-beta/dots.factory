...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    xdg.configFile = {
      "${scripts}/rebuild-dots.sh" = {
        text = # bash
          ''
            ...
            /run/current-system/sw/bin/darwin-rebuild switch --flake "git+ssh://git@github.com/aka-raccoon/dot.factory/#${host}"
          '';
        executable = true;
      };
      ...
      xdg.configFile."${scripts}/authenticate-to-aws.sh" = lib.mkIf config.${namespace}.suits.aws.enable {
        text = # bash
          ''
            #!${pkgs.bash}/bin/bash
