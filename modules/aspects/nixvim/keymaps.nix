{ dots, ... }:
{
  dots.nixvim.includes = [ dots.nixvim._.keymaps ];
  dots.nixvim._.keymaps.homeManager = { ... }: {
  programs.nixvim = {
    ...
    keymaps = [
      ...
      }
      {
        mode = "n";
        key = "<leader>ya";
        action.__raw = "function() yank.yank_path(yank.get_buffer_absolute(), 'absolute') end";
        options = { desc = "Yank absolute path"; };
      }
      {
        mode = "n";
        key = "<leader>yr";
        action.__raw = "function() yank.yank_path(yank.get_buffer_cwd_relative(), 'relative') end";
        options = { desc = "Yank relative path"; };
      }
      {
        mode = "v";
        key = "<leader>ya";
        action.__raw = "function() yank.yank_visual_with_path(yank.get_buffer_absolute(), 'absolute') end";
        options = { desc = "Yank selection with absolute path"; };
      }
      {
        mode = "v";
        key = "<leader>yr";
        action.__raw = "function() yank.yank_visual_with_path(yank.get_buffer_cwd_relative(), 'relative') end";
        options = { desc = "Yank selection with relative path"; };
      }
    ];
  };
  };
